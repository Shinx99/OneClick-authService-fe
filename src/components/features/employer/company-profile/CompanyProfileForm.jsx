...
const CompanyProfileForm = () => {
  ...
  const fetchCompany = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Không thể tải thông tin công ty");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Không thể cập nhật thông tin công ty");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-xl border border-slate-100 p-12 text-center">
        <p className="text-slate-400 font-medium">Đang tải thông tin công ty...</p>
      </div>
    );
  }

  const statusBadge = {
    active: { label: "Đã duyệt", className: "bg-emerald-50 text-emerald-600 border-emerald-200" },
    pending: { label: "Đang chờ duyệt", className: "bg-amber-50 text-amber-600 border-amber-200" },
    rejected: { label: "Đã bị từ chối", className: "bg-rose-50 text-rose-600 border-rose-200" },
  }[status] || null;
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
      ...
      <div className="relative h-44 bg-gradient-to-r from-emerald-800 via-emerald-700 to-teal-600 overflow-hidden">
        {bgUrl && <Image src={bgUrl} alt="Company Background" fill className="object-cover" />}
        <div
        ...
        <div className="absolute -bottom-8 left-6">
          ...
          {uploadingLogo && (
            <p className="text-[10px] text-emerald-600 mt-1 text-center font-medium">Đang tải…</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-6 pt-12 pb-4">
        <div className="flex items-center gap-3">
          <h3 className="text-base font-bold text-slate-700">Thông tin chi tiết</h3>
          {statusBadge && (
            <span
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <input
              ...
              onChange={(e) => handleChange("businessLicenseUrl", e.target.value)}
              disabled={!editMode}
              className={inputClass(!editMode)}
            />
          </div>
          ...
          <div className="col-span-2">
            ...
            <input
              ...
              onChange={(e) => handleChange("financialProofUrl", e.target.value)}
              disabled={!editMode}
              className={inputClass(!editMode)}
            />
          </div>
